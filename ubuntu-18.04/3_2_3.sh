...
function cis_test_run()
{
	...
	cmd=$(grep "net\.ipv4\.conf\.all\.secure_redirects" /etc/sysctl.conf /etc/sysctl.d/* | grep -v "#" | grep "net.ipv4.conf.all.secure_redirects = 0")
	[ -n "$cmd" ] && return 1
	cmd=$(grep "net\.ipv4\.conf\.default\.secure_redirects" /etc/sysctl.conf /etc/sysctl.d/* | grep -v "#" | grep "net.ipv4.conf.default.secure_redirects = 0")
	[ -n "$cmd" ] && return 1
	return 0
}
